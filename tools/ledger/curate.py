...
PICKS = [
    # --- CISA KEV, deadline inside three days ---
    "GitLab CVE-2026-85706 — CVSS 10.0 path traversal",
    'Starlette "BadHost" (CVE-2026-48710) is in CISA KEV',
    "LiteLLM CVE-2026-59822 (MCP authentication bypass",
    # --- CISA KEV, deadline already PASSED ---
    "CVE-2026-21962 remains in CISA KEV and its due date passed",
    "JFrog Artifactory: a second KEV pair",
    # --- actively exploited at scale right now ---
    "AI agent swarms exploited PaperCut NG/MF at scale",
    # --- vendor deadlines inside 7 days ---
    "Entitlement inheritance from the users system group ends tomorrow",
    "Legacy Worksheets and Legacy Dashboards are removed from reader accounts",
    # --- unpatched for someone: this window's dominant CVE shape ---
    "Angular ≤ 19.2.25 is end-of-support",
    "Aurora PostgreSQL has no build carrying the August community security fixes",
    "CVE-2026-85620: postgres-mcp restricted mode is bypassable",
    "Patch now: 8.3.11 / 8.0.32 / 7.0.43",
    # --- patched, but applying the patch is the whole job ---
    "Next.js: two critical unauthenticated RCEs",
    "@7nohe/openapi-react-query-codegen compromised with valid npm provenance",
    # --- a prior edition's open question, resolved: worth as much as a new flag ---
    "parquet-java 1.18.1 is GA, confirmed from the authoritative sources",
]
...
EXCLUDE_ONGOING = [
    "CVE-2026-82067 (CVSS 9.2): authorization can remain disabled",
    "2026-09-14 (tomorrow) — Workspace entitlement behavior enforced",
    "Dry-run the entitlement cutover against a non-prod workspace",
    "2026-09-16 (3 days) — CISA KEV remediation due",
]
