...
def main():
    ...
    # Double-escaping signature is a literal backslash-n AND NO real newlines --
    # exactly the condition the template's md2html repair guard tests. A lone
    # literal backslash-n inside a code span is legitimate brief content and must
    # not fail the build (2026-09-16: aidaily quoted llama.cpp forcing a literal
    # `\n</think>` token, 96 real newlines against 1 literal -- the old assertion
    # would have blocked the publish over correct content).
    for s in back["sections"]:
        if s["md"] and "\\n" in s["md"]:
            assert "\n" in s["md"], (
                "md for %s is double-escaped: literal backslash-n and no real newlines" % s["id"])
