#!/usr/bin/env python3
# SUPERSEDED 2026-09-11 -- kept for reference only, do not run in a build.
# The 09-11 measurement showed post-hoc similarity dedupe of events[] cannot be
# made safe (true duplicates score LOWER on token Jaccard than same-date pairs
# that must never merge). Identity is declared at authoring time instead: see
# ledger_surgery.reuse_key and the hand-verified fold_map*.py files. Preserved
# here because it was the only copy (branch affectionate-maxwell-sehnv0, 09-10)
# and the CLAUDE.md notes reference it by name.
