...
--------------------------------------------------------------------------------
  ...
  - RESILIENCE: if the Artifact call fails with a transient error ("permission
    ...
    allowlisted in the repo .claude/settings.json on both branches, and that is
    sufficient in the current harness (re-proven 2026-08-31: list, read, and the
    lens publish all ran with zero prompts and no user-level settings file).
    NEVER attempt to write ~/.claude/settings.json from a run — writes to Claude
    Code's own config file prompt for manual approval regardless of any
    allowlist (proven three ways on 2026-08-31), so an in-run write can only
    park or spam prompts. If an Artifact call DOES prompt in an unattended run,
    skip 5c per its NON-FATAL rule and fold one line into the notification;
    a "Denied by user" result is a real denial, not plumbing — do not retry it.
...
0. BOOTSTRAP. The VM boots with the repo cloned & GitHub-App-authenticated. Prepare it:
   ...
   (Pages serves the gh-pages root; index.html + codex.html already live there and
    ...
    allow rules. Do NOT write ~/.claude/settings.json from this prompt, ever:
    writes to Claude Code's own config file always prompt for manual approval
    (proven 2026-08-31 — heredoc, Write tool, and sandbox-bypass all prompted),
    so an unattended write can only park the run; and it is unnecessary — the
    repo-level Artifact rule carried the 2026-08-31 lens publish with zero
    prompts. If the harness ever regresses, the fix is the claude.ai
    environment's setup script (boot-time, outside the permission system),
    which only Karl can edit — not an in-run write.)
