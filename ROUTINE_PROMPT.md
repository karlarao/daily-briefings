<!--
========================================================================================
SOURCE-OF-TRUTH COPY OF THE SCHEDULED-ROUTINE PROMPT
========================================================================================
This file is a versioned copy of the instructions that drive the "Daily Briefings"
scheduled routine. The scheduler does NOT read this file — to apply changes you must
copy everything BELOW the horizontal rule and paste it into the routine's prompt/
instructions box in your scheduler config, then save.

CHANGELOG
- 2026-07-06: Added GitHub Pages deploy VERIFICATION + SELF-HEAL. Previously the routine
  treated a successful `git push` as a successful publish. In reality the Pages
  "build and deployment" step fails/stalls intermittently, leaving the live site on the
  PREVIOUS build while the routine reports success and stays silent. The run VM also
  cannot verify by fetching the public URL (egress policy blocks *.github.io). The new
  step 5b polls the deployment status from the GitHub side and re-triggers with an empty
  commit (up to 3x) if it failed or stalled; step 6 notifies if the deploy ultimately
  fails. Edits are confined to: PUBLISH TARGET, RUN PROCEDURE steps 5/5b/6, the workflow
  diagram, and the "publish" note for a future LLM. Everything else is unchanged.
========================================================================================
-->
...
5b. VERIFY THE PAGES DEPLOYMENT (do NOT trust push success alone).
   ...
   the GitHub side:
     - Poll the "pages build and deployment" workflow run whose head_sha == DEPLOY_SHA
       ...
       API, until it reaches a terminal state (allow ≈ up to 3 minutes; do not busy-
   ...
   GitHub API/MCP is unavailable in a headless run), do not assume success — say so in
   the notification.
...
6. NOTIFICATION (exactly one, at the end):
   - If git push FAILED (after retries) OR the Pages deployment FAILED (after
     ...
     was the push or the Pages deploy, and the last DEPLOY_SHA). That failure IS the
     news.
...
<style>
  *,*::before,*::after{box-sizing:border-box}
  :root{
    ...
    --ok:#2f855a;--urgent:#c0392b;--slow:#7b8798;
  ...
  @media (prefers-color-scheme:dark){:root{
    ...
    --ok:#4cc487;--urgent:#f0685b;--slow:#6d7a8b;--shadow:0 1px 2px rgba(0,0,0,.4),0 10px 34px rgba(0,0,0,.45);
  }}
  :root[data-theme="light"]{
    ...
    --ok:#2f855a;--urgent:#c0392b;--slow:#7b8798;--shadow:0 1px 2px rgba(16,29,41,.06),0 8px 28px rgba(16,29,41,.07);
  }
  ...
  .sdot.ok{background:var(--ok)}.sdot.urgent{background:var(--urgent)}.sdot.pending{background:var(--surface-3);border:1px solid var(--border)}.sdot.slow{background:var(--slow)}
  ...
  .pending{color:var(--ink-faint);font-style:italic;padding:20px 0}
